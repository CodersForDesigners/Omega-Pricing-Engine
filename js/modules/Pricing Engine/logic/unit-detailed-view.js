...
$( document ).on( "unit/view", function ( event, data ) {
	...
	// Get the context under which this event was triggered
	// If it was a modification ( specifically, a binary one ),
		// then wait for teeny bit ( 'cause animation )
	var waitTimeForAnimation = 0;
	if ( newUnitData && newUnitData.Unit == void 0 )
		waitTimeForAnimation = 0.31;	// 350 milli-seconds

	waitFor( waitTimeForAnimation ).then( function () {
